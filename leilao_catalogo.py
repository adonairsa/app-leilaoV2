...
def obter_api_keys():
    ...
    # Tenta ler dos secrets
    try:
        if "DEEPSEEK_API_KEY" in st.secrets:
            chaves_deepseek.append(st.secrets["DEEPSEEK_API_KEY"])
        if "ANTHROPIC_API_KEY" in st.secrets:
            chaves_anthropic.append(st.secrets["ANTHROPIC_API_KEY"])
    except:
        pass
    ...
    if not chaves_deepseek:
        ...
        if env_ds:
            chaves_deepseek.append(env_ds)
    
    if not chaves_anthropic:
        env_ant = os.environ.get("ANTHROPIC_API_KEY", "")
        if env_ant:
            chaves_anthropic.append(env_ant)
    
    return chaves_deepseek, chaves_anthropic
...
def encontrar_pagina_catalogo_auto(texto_cat_tuple, num_lote, dados_lote):
    """
    Auto-detecção da página correta do catálogo usando múltiplas estratégias
    """
    ...
    # Estratégia 1: Procura "LOTE XX" ou "LT XX" no texto
    if num_clean:
        n_int = int(num_clean)
        padroes_lote = [
    ...
    if nome_animal:
        ...
        if palavras:
            # Procura páginas que contenham o nome
            melhores_matches = []
            for idx, pagina in enumerate(texto_cat):
                if pagina:
                    ...
                    if matches > 0:
                        melhores_matches.append((matches, idx, pagina))
            
            if melhores_matches:
                # Ordena por número de matches (melhor primeiro)
                melhores_matches.sort(key=lambda x: x[0], reverse=True)
                return melhores_matches[0][1], melhores_matches[0][2]
...
def extrair_texto_imagem_claude(img_bytes, ant_keys):
    """Claude lê a imagem e transcreve o texto"""
    ...
    payload = {
        ...
        "messages": [{
            ...
            "content": [
                ...
                {
                    "type": "text",
                    "text": """Transcreva TODO o texto desta página do catálogo do leilão.
                    Inclua:
                    - Nome do animal
                    - Número do lote
                    - Espécie/Raça
                    ...
                    Formato: Texto simples, linha por linha."""
                }
            ]
        }]
    }
    ...
    for api_key in ant_keys:
        ...
        try:
            ...
            if response.status_code == 200 and 'content' in res_json:
                ...
                st.warning(f"Erro Claude: {res_json.get('error', {}).get('message', 'Erro desconhecido')}")
                
        except Exception as e:
            st.warning(f"Erro conexão Claude: {str(e)}")
            continue
    
    return ""

# ==================== ANÁLISE COM DEEPSEEK ====================
def analisar_com_deepseek(num_lote, dados_lote, texto_catalogo, texto_ocr_claude, ds_keys):
    """DeepSeek analisa tudo e gera resposta final"""
    if not ds_keys:
        return None, "⚠️ Configure DEEPSEEK_API_KEY no secrets.toml"
    
    prompt_system = """Você é um Leiloeiro Rural de Elite.
    Analise os dados do animal e do catálogo para criar uma apresentação perfeita.
    
    REGRAS:
    1. Use linguagem de leilão profissional
    2. Destaque os pontos fortes
    3. Crie gatilhos emocionais
    4. Seja agressivo na venda
    5. Respeite a espécie do animal
    
    Retorne JSON com:
    {
        "nome_animal": "...",
        "especie": "equino/bovino_corte/bovino_leite/muar",
        "especie_emoji": "🐴/🐂/🐄/🫏",
        "encartes": [{"titulo": "...", "valor": "..."}],
        "apresentacao": "frase de venda",
        "genetica_pai": "...",
        "genetica_mae": "...",
        "reproducao": "...",
        "gatilhos": ["...", "...", "..."]
    ...
    for api_key in ds_keys:
        ...
        try:
            ...
            if response.status_code == 200 and 'choices' in res_json:
                ...
                erro = res_json.get('error', {}).get('message', 'Erro desconhecido')
                return None, f"Erro DeepSeek: {erro}"
                
        except Exception as e:
            return None, f"Erro conexão DeepSeek: {str(e)}"
    
    return None, "Nenhuma chave DeepSeek funcionou"
...
def run():
    ...
    with st.sidebar:
        ...
        if ds_keys:
            st.markdown(f'<div class="status-box status-ok">✅ DeepSeek: {len(ds_keys)} chave(s)</div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="status-box status-error">❌ DeepSeek: Não configurado</div>', unsafe_allow_html=True)
        
        if ant_keys:
            st.markdown(f'<div class="status-box status-ok">✅ Claude: {len(ant_keys)} chave(s)</div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="status-box status-error">❌ Claude: Não configurado</div>', unsafe_allow_html=True)
    ...
    # Navegação
    ordem_texto = f"{modo_ordenacao} | Lote {st.session_state.lote_idx + 1} de {len(lista_lotes)}"
    ...
    if texto_cat and file_bytes_cat:
        ...
        if pagina_detectada >= 0:
            st.success(f"📖 Página {pagina_detectada + 1} do catálogo detectada automaticamente")
        else:
            st.warning("⚠️ Página do catálogo não detectada automaticamente")
    
    # Obtém imagem da página detectada
    ...
    with col_esquerda:
        ...
        with st.spinner("🤖 Analisando com IA..."):
            ...
            if ds_keys:
                ...
                st.warning("⚠️ Configure DEEPSEEK_API_KEY no secrets.toml para análise completa")
    
    with col_direita:
        # Mostra imagem do catálogo
        if img_pagina_bytes:
            st.image(img_pagina_bytes, use_container_width=True, caption=f"Catálogo - Página {pagina_detectada + 1}")
        
        # Mostra texto OCR do Claude
        if texto_ocr_claude:
        ...
        if texto_pagina_catalogo:
            with st.expander("📄 Texto extraído do PDF"):
                st.text(texto_pagina_catalogo[:2000])
