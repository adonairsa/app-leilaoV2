...
def pre_carregar_lotes_vizinhos(file_bytes_cat, lista_lotes, idx_atual, mapa_oe, indice_catalogo, qtd=QTD_PRE_CARREGAR):
    if not file_bytes_cat or not indice_catalogo:
        return
    paginas_vizinhas = []
    offsets = list(range(-qtd, 0)) + list(range(1, qtd + 1))
    for offset in offsets:
        idx_vizinho = idx_atual + offset
        if idx_vizinho < 0 or idx_vizinho >= len(lista_lotes):
            continue
        lt_vizinho = lista_lotes[idx_vizinho]
        dados_lt = mapa_oe.get(lt_vizinho, {})
        dados_cat = encontrar_no_indice(lt_vizinho, dados_lt.get("nome_animal", ""), indice_catalogo)
        if dados_cat:
            pagina = dados_cat.get("_pagina", -1)
            if pagina >= 0:
                paginas_vizinhas.append(pagina)
    if paginas_vizinhas:
        threading.Thread(target=_prefetch_paginas_worker, args=(file_bytes_cat, paginas_vizinhas), daemon=True).start()
...
# ==================== CLAUDE INDEXA LOTE DE PÁGINAS (BATCH) ====================
def claude_indexar_lote_paginas(imagens_bytes, _ant_keys):
    ...
    Para CADA página, extraia (se for ficha de lote):
    ...
    Se a página NÃO for ficha de lote, retorne "numero_lote": null.
    ...
    O array "paginas" DEVE ter exatamente {len(imagens_bytes)} itens.
    """
...
# ==================== DEEPSEEK GERA ABERTURA/GATILHOS ====================
def deepseek_processar_lotes_batch(lotes_info, _ds_keys):
    if not _ds_keys or not lotes_info:
        return {}, "sem chave ou sem lotes"

    prompt = f"""
    Você é um locutor de leilão. Para CADA lote abaixo, gere material de apoio.

    LOTES (JSON):
    {json.dumps(lotes_info, ensure_ascii=False, indent=2)}

    Para cada lote gere:
    - "abertura": frase curta (máx. 25 palavras)
    - "encartes": lista com 3-4 dados importantes
    - "gatilhos": 3 gatilhos curtos
    - "observacao_destaque": 1 frase ou ""
